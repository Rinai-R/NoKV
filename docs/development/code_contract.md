...
| `crates/nokv-agent/` | Agent runtime and index primitives: agent-local store traits, Holt-backed event records, event ingestion, secondary indexes, coverage state, typed query execution, and model/tool-facing agent protocols over agent-native records. | Own NoKV DFS metadata semantics, inode/dentry layout, object-reference GC, FUSE/kernel cache policy, metadata RPC routing, expose DFS namespace tools, or require NoKV DFS crates. |
